...
class SolverGUI(Frame):
	...
	def create_gui(self):
		...
		for i in xrange(9):
			for j in xrange(9):
				...
				cell_specifier = board_canvas.create_rectangle(cell_size*j, cell_size*i, cell_size*(j+1), cell_size*(i+1), fill="#333333", width=0)
				board_canvas.tag_bind(cell_specifier, "<Button>", self.cell_pressed)
				
				# Set the font and create the text field
				font = tkFont.Font(family='Arial', size=20)
				textSpecifier = board_canvas.create_text(cell_size*(j+0.5), cell_size*(i+0.5), fill="#CCCCCC", font=font, state=DISABLED)
		...
		for i in xrange(10):
			...
			# Draw the lines
			horizontal_specifier = board_canvas.create_line(cell_size*i, 0, cell_size*i, canvas_size, fill=color, width=width)
			vertical_specifier = board_canvas.create_line(0, cell_size*i, canvas_size, cell_size*i, fill=color, width=width)
		...
		status_bar = Message(self, text="Click a cell and type a number! You'll need at least 17.", width=360, background="#444444", foreground="#CCCCCC")
		self.status_bar = status_bar
		status_bar.place(x=20, y=400, width=360, height=30)
		# Create the solve button
		solve_button = Button(self, text="Solve", command=self.solve_button_pressed, highlightbackground="#222222")
		self.solve_button = solve_button
		solve_button.place(x=20, y=450, width=170, height=20)
		# Create the reset button
		reset_button = Button(self, text="Reset", command=self.reset_button_pressed, highlightbackground="#222222")
	...
	def cell_pressed(self, event):
		...
		self.selected_text_specifier = self.text_specifiers[self.selected_cell_specifier]
	...
	def key_pressed(self, event):
		...
		if self.selected_text_specifier is not None:
			...
			if event.char in "123456789" and event.char != "":
				self.canvas.itemconfig(self.selected_text_specifier, text=event.char)
				index = self.cell_indices[self.selected_text_specifier]
				# Update the model to reflect the changes
				self.board_string = self.board_string[:index] + event.char + self.board_string[index+1:]
			# Delete the text if the user pressed the backspace or delete keys
				...
				index = self.cell_indices[self.selected_text_specifier]
				# Update the model to reflect the changes
				self.board_string = self.board_string[:index] + "0" + self.board_string[index+1:]
	...
	def solve_button_pressed(self):
		...
		if not(board_model.is_solvable()):
			self.status_bar.config(text="No solution. Do you have 17+ consistent numbers?")
			return
		# Attempt a solution
		board_model.try_solve()
		# Check for any inconsistencies
		if not(board_model.was_solved()):
			self.status_bar.config(text="Oops, looks like your numbers are inconsistent.")
			return
	...
	def reset_button_pressed(self):
		...
		# Update the status bar
		self.status_bar.config(text="Click a cell and type a number! You'll need at least 17.")
